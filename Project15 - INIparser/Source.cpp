	...
	for (const auto& line : this->input)
	{
		if (std::regex_match(line, res, sectionRegEx))
		{
			...
			if (this->data.find(sectionName) != this->data.end())
				cout << "The value of the section name is repeated\n";
		...
		else if (std::regex_match(line, res, paramRegEx))
		{
			...
			if (this->data.find(sectionName) == this->data.end())
			{
				...
				if (!(this->data[sectionName].insert(std::make_pair(paramName, paramValue)).second))
					cout << "The value of the parameter name is repeated\n";
			}
		}
		else
		{
...
int main()
{
	...
	my_iniparser.load("C:/Users/Boss/Desktop/INIparser/test1.ini");
